  ...
  for (int i=0; i<8; i++){
    ...
    if (iM < M){
      for (int j=0; j<8; j++){
        ...
        if (iN < N){
          uint8_t element_mask = ElementMask[(__MASK_BATCH__)*M*N + (iM)*N + (iN)];
...
__global__ void mbmm_tn(
  ...
  int tM = (M + 8 - 1) / 8;
  int tN = (N + 8 - 1) / 8;
  uint8_t block_mask = BlockMask[__MASK_BATCH__*bM*bN + (blockIdx.z)*bN + (blockIdx.y)];
  uint8_t thread_mask = ThreadMask[__MASK_BATCH__*tM*tN + (blockIdx.z*16 + vy)*tN + (blockIdx.y*16 + vx) ];
  if (block_mask == 0)
    return;
