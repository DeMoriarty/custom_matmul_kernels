...
We can see that MBMM is nearly 100% faster that the custom BMM + masked_fill, and around 5 ~ 10% faster than torch.bmm + masked_fill. The mask used above has about 50% sparsity. It gets faster when the mask is even more sparse.  
...
```
Normally there isn't a problem doing this, however, when optimizing my implementation of K-means, this become an issue. One of the main steps of K-means algorithm is computing the distance between every data point and every centroid (cluster center), and get index of the closest cluster for each data point (argmin).  
...
2. No need for loop  
3. Possibily faster than matmul then argmin (less memory ops).  

The K-Means implementation is in [here](https://github.com/DeMoriarty/TorchPQ/tree/main/torchpq/kmeans), 
And [this](https://github.com/DeMoriarty/TorchPQ/blob/main/torchpq/kmeans/kernels/MaxSimKernel.cu) is the fused kernel. Maybe I will put them in this repo as well.  

## Sampled Softmax
There is a family of softmax approximations called sampled softmax.
